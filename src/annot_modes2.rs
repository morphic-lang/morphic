...
use crate::data::mode_annot_ast2::{
    self as annot, Lt, LtParam, Mode, ModeLowerBound, ModeParam, Occur, Overlay, OverlaySubst,
    TypeSubst,
};
use crate::intrinsic_config::intrinsic_sig;
use crate::pretty_print::graphviz as gv;
...
fn count_mode_params<'a, A: 'a, B: 'a, C: 'a, D: 'a>(
    ...
    ty.map(
        &|id, _| customs.get(id).unwrap().num_mode_params.to_value(),
        &|_| 1,
...
// Invariant: must produce `ModeParam`s in the same order that they are consumed by
// `parameterize_overlay`.
fn find_overlay_params(ty: &annot::Type<ModeParam, LtParam>) -> Vec<ModeParam> {
    fn find_overlay_params_rec(ty: &annot::Type<ModeParam, LtParam>, params: &mut Vec<ModeParam>) {
        match ty {
            annot::Type::Bool | annot::Type::Num(_) => {}
            annot::Type::Tuple(tys) => {
                for ty in tys {
                    find_overlay_params_rec(ty, params);
                }
            }
            ...
            annot::Type::Custom(_, subst) => {
                for m in subst.os.0.values() {
                    params.push(*m);
                }
            }
            annot::Type::Array(m, _, _, _)
            | annot::Type::HoleArray(m, _, _, _)
            | annot::Type::Boxed(m, _, _, _) => {
                params.push(*m);
            }
        }
    }
    ...
    find_overlay_params_rec(ty, &mut params);
    params
}

// Invariant: must consume `modes_to_use` in the same order that they are produced by
// `find_overlay_params`.
fn parameterize_overlay(
    parameterized: &IdMap<CustomTypeId, annot::TypeDef>,
    modes_to_use: &Vec<ModeParam>,
    sccs: &IdVec<CustomTypeId, CustomTypeSccId>,
    self_id: CustomTypeSccId,
    ty: &anon::Type,
) -> Overlay<ModeParam> {
    let modes_to_use = RefCell::new(modes_to_use.clone());
    ty.overlay_items(Some((sccs, self_id)))
        .map(
            &|id, _| {
                OverlaySubst(
                    parameterized[id]
                        .overlay_params
                        .iter()
                        .map(|p| (*p, modes_to_use.borrow_mut().pop().unwrap()))
                        .collect(),
                )
            },
            &|_| modes_to_use.borrow_mut().pop().unwrap(),
        )
        .collect()
}
            ...
            (
                *id,
                annot::TypeDef {
                    num_mode_params,
                    num_lt_params,
                    overlay_params: overlay_params.into_iter().collect(),
                    content,
                    overlay,
                },
            )
...
fn mode_bind<L1, L2>(
    ...
    ty1.items().zip(ty2.items()).for_each(
        &|_, (subst1, subst2)| {
            for (m1, m2) in subst1.ms.values().zip_eq(subst2.ms.values()) {
                constrs.borrow_mut().require_eq(*m1, *m2);
            }
            for (m1, m2) in subst1.os.0.values().zip_eq(subst2.os.0.values()) {
                constrs.borrow_mut().require_eq(*m1, *m2);
            }
        },
        &|((m1, _), (m2, _))| {
...
/// Given *heap* types `ty1` and `ty2` mode bind only their storage modes (i.e. ignore the modes in
/// overlays on `Array`, `HoleArray`, and `Boxed`)
fn mode_bind_storage<L1, L2>(
    constrs: &mut ConstrGraph,
    ty1: &annot::Type<ModeVar, L1>,
    ty2: &annot::Type<ModeVar, L2>,
) {
    ...
    match (binding_ty, fut_ty, binding_ov, fut_ov) {
        ...
        (
            ...
            for (((ty1, ty2), o1), o2) in tys1.iter().zip_eq(tys2).zip_eq(os1).zip_eq(os2) {
                emit_occur_constrs_overlay(customs, constrs, path, ty1, ty2, o1, o2);
            }
        }
        (
        ...
        (
            annot::Type::Custom(id1, subst1),
            annot::Type::Custom(id2, subst2),
            annot::Overlay::Custom(id3, subst3),
            annot::Overlay::Custom(id4, subst4),
        ) if id1 == id2 && id1 == id3 && id1 == id4 => {
            ...
            emit_occur_constrs_overlay(
                ...
                constrs,
                path,
                &subst1.apply(&typedef.content),
                &subst2.apply(&typedef.content),
                &subst3.apply(&typedef.overlay),
                &subst4.apply(&typedef.overlay),
            );
        }
        (
    ...
    match (binding_ty, fut_ty) {
        ...
        (annot::Type::Tuple(tys1), annot::Type::Tuple(tys2)) => {
            for (ty1, ty2) in tys1.iter().zip_eq(tys2) {
                emit_occur_constrs_heap(customs, constrs, path, ty1, ty2);
            }
        }
        ...
        (annot::Type::Custom(id1, subst1), annot::Type::Custom(id2, subst2)) if id1 == id2 => {
            for (m1, m2) in subst1.os.0.values().zip_eq(subst2.os.0.values()) {
...
fn emit_occur_constrs(
    ...
    match (binding_ty, fut_ty) {
        ...
        (annot::Type::Tuple(tys1), annot::Type::Tuple(tys2)) => {
            for (ty1, ty2) in tys1.iter().zip_eq(tys2) {
                emit_occur_constrs(customs, constrs, path, ty1, ty2);
            }
        }
        (annot::Type::Variants(tys1), annot::Type::Variants(tys2)) => {
            for ((_, ty1), (_, ty2)) in tys1.iter().zip_eq(tys2) {
        ...
        (annot::Type::Custom(id1, subst1), annot::Type::Custom(id2, subst2)) if id1 == id2 => {
            let typedef = &customs[id1];
            let binding_ty = subst1.apply(&typedef.content);
            let binding_ov = subst1.os.apply(&typedef.overlay);
            let fut_ty = subst2.apply(&typedef.content);
            let fut_ov = subst2.os.apply(&typedef.overlay);
            emit_occur_constrs(customs, constrs, path, &binding_ty, &fut_ty);
            emit_occur_constrs_overlay(
                customs,
                constrs,
                path,
...
fn same_shape(ty1: &anon::Type, ty2: &SolverType) -> bool {
    match (ty1, ty2) {
        ...
        (anon::Type::Custom(id1), annot::Type::Custom(id2, _)) => id1 == id2,
    ...
    ty.items()
        .map(
            &|_, subst| TypeSubst {
                ms: subst.ms.clone(),
                os: subst.os.clone(),
                lts: subst.lts.map_refs(|_, lt| do_subst(lt)),
            },
            &|(m, lt)| (*m, do_subst(lt)),
            &|_, ms| ms.clone(),
            &|m| *m,
    ...
    ty.items()
        .map(
            &|_, subst| TypeSubst {
                ms: subst.ms.clone(),
                os: subst.os.clone(),
                lts: subst.lts.map_refs(|_, lt| Lt::var(*lt)),
            },
            &|(m, lt)| (*m, Lt::var(*lt)),
            &|_, ms| ms.clone(),
            &|m| *m,
    ...
    ty.items()
        .map(
            &|_, subst| TypeSubst {
                ms: subst.ms.clone(),
                os: subst.os.clone(),
                lts: subst.lts.map_refs(|_, lt| lt.join(new_lt)),
            },
...
fn lt_equiv(ty1: &annot::Type<ModeVar, Lt>, ty2: &annot::Type<ModeVar, Lt>) -> bool {
    ty1.items()
        .zip(ty2.items())
        .map(
            &|_, (subst1, subst2)| subst1.lts == subst2.lts,
            &|((_, lt1), (_, lt2))| lt1 == lt2,
            &|_, _| true,
            &|_| true,
        )
...
fn apply_overlay<L: Clone>(
    ...
    match (ty, overlay) {
        ...
        (annot::Type::Custom(id1, subst1), annot::Overlay::Custom(id2, subst2)) if id1 == id2 => {
            annot::Type::Custom(
                *id1,
                TypeSubst {
                    ms: subst1.ms.clone(),
                    os: subst2.clone(),
                    lts: subst1.lts.clone(),
                },
            )
        }
...
fn unapply_overlay<L>(
    ...
    match ty {
        ...
        annot::Type::Custom(id, subst) => annot::Overlay::Custom(*id, subst.os.clone()),
...
fn compute_overlay_subst<'a>(
    ...
    params.zip(vars).for_each(
        &|_, (params, ms)| {
            for (param1, (param2, m)) in params.0.keys().zip(&ms.0) {
...
fn unfold_overlay(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    overlay: &SolverOverlay,
    self_subst: &OverlaySubst<ModeVar>,
) -> SolverOverlay {
    match overlay {
        ...
        annot::Overlay::Tuple(os) => annot::Overlay::Tuple(
            os.iter()
                .map(|o| unfold_overlay(customs, o, self_subst))
                .collect(),
        ),
        annot::Overlay::Variants(os) => {
            annot::Overlay::Variants(os.map_refs(|_, o| unfold_overlay(customs, o, self_subst)))
        }
        annot::Overlay::SelfCustom(id) => annot::Overlay::Custom(*id, self_subst.clone()),
        annot::Overlay::Custom(id, os) => annot::Overlay::Custom(*id, os.clone()),
...
fn unfold_type<L: Clone>(
    ...
    ty: &annot::Type<ModeVar, L>,
    self_subst: &TypeSubst<ModeVar, L>,
) -> annot::Type<ModeVar, L> {
    match ty {
        annot::Type::Bool => annot::Type::Bool,
        ...
        annot::Type::Tuple(tys) => annot::Type::Tuple(
            tys.iter()
                .map(|ty| unfold_type(customs, extracted, ty, self_subst))
                .collect(),
        ),
        annot::Type::Variants(tys) => annot::Type::Variants(
            tys.map_refs(|_, ty| unfold_type(customs, extracted, ty, self_subst)),
        ),
        annot::Type::SelfCustom(id) => annot::Type::Custom(
            *id,
            TypeSubst {
                ms: self_subst.ms.clone(),
                os: extracted.clone(),
                lts: self_subst.lts.clone(),
            },
        ),
        annot::Type::Custom(id, subst) => annot::Type::Custom(*id, subst.clone()),
        annot::Type::Array(m, lt, ty, o) => annot::Type::Array(
            m.clone(),
            lt.clone(),
            Box::new(unfold_type(customs, extracted, ty, self_subst)),
            unfold_overlay(customs, o, &self_subst.os),
        ),
...
fn unfold_custom<L: Clone>(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    subst: &TypeSubst<ModeVar, L>,
    id: CustomTypeId,
) -> annot::Type<ModeVar, L> {
    let typedef = &customs[id];
    let folded = subst.apply(&typedef.content);
    let extracted = compute_overlay_subst(typedef.overlay.items(), folded.overlay_items());
    let unfolded = unfold_type(customs, &extracted, &folded, &subst);
    unfolded
}
    ...
    ty.map(
        &|id, _| {
            ...
            TypeSubst {
                ...
                os: fresh_overlay_subst(*constrs.borrow_mut(), &typedef.overlay_params),
...
fn instantiate_occur(
    ...
    emit_occur_constrs(customs, constrs, path, old_ty, fut_ty);
...
fn replace_modes<M1: Id, M2, L: Clone>(
    ...
    let lookup = |m| remapping.borrow_mut()(m);
    ty.items()
        .map(
            &|_, subst| TypeSubst {
                ms: subst.ms.map_refs(|_, m| lookup(*m)),
                os: OverlaySubst(subst.os.0.iter().map(|(p, m)| (*p, lookup(*m))).collect()),
                lts: subst.lts.clone(),
            },
            &|(m, lt)| (lookup(*m), lt.clone()),
            &|_, subst| OverlaySubst(subst.0.iter().map(|(p, m)| (*p, lookup(*m))).collect()),
            &|m| lookup(*m),
        )
        .collect()
}
...
impl<'a> SignatureAssumptions<'a> {
    fn sig_of(
        ...
        self.known_defs.get(id).map_or_else(
            ...
            |def| {
                let params_to_vars = constrs.instantiate_subgraph(&def.constrs);
...
fn instantiate_expr(
    ...
    let expr_annot = match expr {
        flat::Expr::Local(local) => {
            let occur = instantiate_occur(customs, &mut ctx, constrs, &path, *local, fut_ty);
        ...
        flat::Expr::Call(purity, func, arg) => {
            ...
            // This `join_everywhere` arises from the fact that we assume that functions access all
            // of their arguments. We could be more precise here.
            let arg_ty = join_everywhere(&replace_lts(&arg_ty, &lt_subst), &path.seq(1).as_lt());
            println!("arg_ty: {:?}", arg_ty);
            let arg = instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arg, &arg_ty);
        ...
        flat::Expr::LetMany(bindings, result_id) => scopes.with_scope(|scopes| {
            ...
            // println!("-------------------");
            for (binding_ty, _binding_expr) in bindings {
                ...
                // println!("${} : {:?} = {:?}", local_id1.0, binding_ty, binding_expr);
            }
            // println!("in ${} as {:?}", result_id.0, fut_ty);
            // println!("+++++++++++++++++++");
            ...
            for (i, (_, binding_expr)) in bindings.iter().enumerate().rev() {
                ...
                for (id, _) in &expr_updates.0 {
                    if !ctx.updates.0.contains_key(id) {
                        println!("---------------------");
                        println!("id: {:?}", id);
                        println!("expr_annot: {:?}", expr_annot);
        ...
        flat::Expr::Tuple(item_ids) => {
            ...
            let mut occurs_rev: Vec<_> = item_ids
                ...
                // modifies the lifetimes in `ctx`) generates the correct constraints.
                .rev()
        ...
        flat::Expr::TupleField(tuple_id, idx) => {
            ...
            // println!("before: {:?}", &**ctx.local_binding(*tuple_id));
            ...
            // println!("tuple_id: {}, tuple_ty: {:?}", tuple_id.0, tuple_ty);
            // println!("after: {:?}", &**ctx.local_binding(*tuple_id));
        ...
        flat::Expr::UnwrapVariant(variant_id, wrapped) => {
            let annot::Type::Variants(binding_variant_tys) = &**ctx.local_binding(*wrapped) else {
                unreachable!();
            };

            let wrapped_ty = annot::Type::Variants(binding_variant_tys.map_refs(|id, ty| {
                if id == *variant_id {
                    fut_ty.clone()
                } else {
                    ty.clone()
        ...
        flat::Expr::UnwrapBoxed(wrapped, _item_ty) => {
            ...
            let box_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *wrapped, &box_ty);

            annot::Expr::UnwrapBoxed(box_occur, item_ty)
        }

        flat::Expr::WrapCustom(custom_id, content) => {
            let annot::Type::Custom(fut_id, fut_subst) = fut_ty else {
                unreachable!();
            };

            debug_assert_eq!(*custom_id, *fut_id);

            let unfolded_ty = unfold_custom(customs, &fut_subst, *custom_id);
            let unfolded_ov = &unfold_overlay(
                customs,
                &fut_subst.os.apply(&customs[*custom_id].overlay),
                &fut_subst.os,
            );
        ...
        flat::Expr::UnwrapCustom(custom_id, folded) => {
            ...
            let folded_ov = instantiate_overlay(customs, constrs, custom.overlay.items());
            let folded_os = compute_overlay_subst(custom.overlay.items(), folded_ov.items());
            ...
            let unfolded_ty = apply_overlay(
                &unfold_custom(customs, &folded_ts, *custom_id),
                &unfold_overlay(customs, &folded_ov, &folded_os),
            );

            mode_bind(constrs, &unfolded_ty, fut_ty);

            let folded_ty = replace_lts(
                &wrap_lt_params(&annot::Type::Custom(*custom_id, folded_ts)),
        ...
        flat::Expr::ArrayOp(flat::ArrayOp::Get(_, arr_id, idx_id)) => {
            ...
            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, &arr_ty);
        ...
        flat::Expr::ArrayOp(flat::ArrayOp::Extract(item_ty, arr_id, idx_id)) => {
            ...
            mode_bind_storage(constrs, &annot_item_ty, fut_item_ty);
        ...
        flat::Expr::ArrayOp(flat::ArrayOp::Len(item_ty, arr_id)) => {
            ...
            let item_ty = instantiate_type_unused(customs, constrs, item_ty);
            let arr_ty = annot::Type::Array(
                constrs.fresh_var(),
                path.seq(1).as_lt(),
                Box::new(item_ty.clone()),
                instantiate_overlay(customs, constrs, item_ty.overlay_items()),
            );
        ...
        flat::Expr::ArrayOp(flat::ArrayOp::Push(item_ty, arr_id, item_id)) => {
            ...
            let arr_item_ty = replace_modes(|_| constrs.fresh_var(), fut_item_ty);
            mode_bind_storage(constrs, &arr_item_ty, fut_item_ty);

            let pushed_item_ty = replace_modes(|_| constrs.fresh_var(), fut_item_ty);
            mode_bind_storage(constrs, &pushed_item_ty, fut_item_ty);

            let arr_ty = annot::Type::Array(
                arr_mode,
                path.seq(1).as_lt(),
                Box::new(pushed_item_ty.clone()),
                instantiate_overlay(customs, constrs, item_ty.overlay_items(None)),
            );
            ...
            annot::Expr::ArrayOp(annot::ArrayOp::Push(pushed_item_ty, arr_occur, item_occur))
        }
        ...
        flat::Expr::ArrayOp(flat::ArrayOp::Pop(item_ty, arr_id)) => {
            ...
            let popped_item_ty = &ret_tys[1];

            require_owned(constrs, *fut_arr_mode);
            mode_bind_storage(constrs, popped_item_ty, fut_item_ty);
            ...
            let arr_ty = annot::Type::Array(
                arr_mode,
                path.seq(1).as_lt(),
                Box::new(popped_item_ty.clone()),
                instantiate_overlay(customs, constrs, item_ty.overlay_items(None)),
            );

            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, &arr_ty);
            annot::Expr::ArrayOp(annot::ArrayOp::Pop(popped_item_ty.clone(), arr_occur))
        }
        ...
        flat::Expr::ArrayOp(flat::ArrayOp::Replace(item_ty, hole_id, item_id)) => {
            ...
            let hole_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *hole_id, &hole_ty);
        ...
        flat::Expr::ArrayOp(flat::ArrayOp::Reserve(_item_ty, arr_id, cap_id)) => {
            ...
            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, fut_ty);
        ...
        flat::Expr::IoOp(flat::IoOp::Input) => {
            let annot::Type::Array(fut_mode, _, item_ty, _) = fut_ty else {
                unreachable!();
            };
        ...
        flat::Expr::Panic(ret_ty, msg_id) => {
            let msg_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(1), *msg_id, fut_ty);
            let ret_ty = instantiate_type_unused(customs, constrs, ret_ty);
...
fn extract_type(solution: &Solution, ty: &SolverType) -> annot::Type<ModeLowerBound, Lt> {
    let map_os = |os: &OverlaySubst<ModeVar>| {
        OverlaySubst(
            os.0.iter()
                .map(|(p, m)| (*p, solution.lower_bounds[*m].clone()))
                .collect(),
        )
    };
    ty.items()
        .map(
            &|_, subst| TypeSubst {
                ms: subst.ms.map_refs(|_, m| solution.lower_bounds[*m].clone()),
                os: map_os(&subst.os),
                lts: subst.lts.clone(),
            },
            &|(m, lt)| (solution.lower_bounds[*m].clone(), lt.clone()),
            &|_, subst| map_os(subst),
            &|m| solution.lower_bounds[*m].clone(),
        )
        .collect()
}

fn extract_occur(solution: &Solution, occur: &SolverOccur) -> annot::Occur<ModeLowerBound, Lt> {
    annot::Occur {
        local: occur.local,
...
) -> annot::Condition<ModeLowerBound, Lt> {
    match cond {
...
fn extract_expr(solution: &Solution, expr: &SolverExpr) -> annot::Expr<ModeLowerBound, Lt> {
    use annot::Expr as E;
    match expr {
...
fn solve_scc(
    ...
    // let vars = &instantiated.scc_constrs.vars;
    // let sccs: Sccs<SccId, _> = find_components(vars.count(), |var| &vars[var].lb_vars);
    ...
    //     let mut buf = Vec::new();
    //     let mut w = gv::GraphvizWriter::new(2);
    //     w.write_digraph(&mut buf, |buf, w| {
    //         let scc = sccs.component(my_id);

    //         let borrowed = w.write_node(buf, "&", None).unwrap();
    //         let owned = w.write_node(buf, "!", None).unwrap();
    //         let nodes: BTreeMap<_, _> = scc
    //             .nodes
    //             .iter()
    //             .map(|var| (var, w.write_node(buf, &format!("{}", var.0), None).unwrap()))
    //             .collect();
    //         for var in scc.nodes {
    //             let var_constrs = &instantiated.scc_constrs.vars[*var];
    //             for other in &var_constrs.lb_vars {
    //                 w.write_edge(buf, nodes[other], nodes[&var], None).unwrap();
    //             }
    //             match var_constrs.lb_const {
    //                 Mode::Borrowed => {
    //                     w.write_edge(buf, borrowed, nodes[&var], None).unwrap();
    //                 }
    //                 Mode::Owned => {
    //                     w.write_edge(buf, owned, nodes[&var], None).unwrap();
    //                 }
    //             }
    //         }
    //         Ok(())
    //     })
    //     .unwrap();
    //     let mut file = std::fs::File::create(format!("graph{}.dot", scc.nodes[0].0)).unwrap();
