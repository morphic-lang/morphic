use crate::data::anon_sum_ast as anon;
use crate::data::first_order_ast::{CustomFuncId, CustomTypeId, NumType};
use crate::data::flat_ast::{self as flat, CustomTypeSccId, LocalId};
use crate::data::mode_annot_ast2::{
    self as annot, Lt, LtParam, Mode, ModeConstr, ModeParam, Overlay,
};
use crate::util::graph as old_graph; // TODO: switch completely to `id_graph_sccs`
use crate::util::iter::IterExt;
use crate::util::local_context::LocalContext;
use crate::util::map_ext::MapExt;
use crate::util::progress_logger::{ProgressLogger, ProgressSession};
use id_collections::{id_type, Count, Id, IdMap, IdVec};
use id_graph_sccs::{find_components, Scc, SccKind, Sccs};
use std::cell::{RefCell, RefMut};
use std::collections::{BTreeMap, BTreeSet};
use std::iter;
use std::rc::Rc;

struct State<T>(Rc<RefCell<T>>);

impl<T> Clone for State<T> {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}

fn state<T>(inner: T) -> State<T> {
    State(Rc::new(RefCell::new(inner)))
}

impl<T> State<T> {
    fn get<'a>(&'a self) -> RefMut<'a, T> {
        self.0.borrow_mut()
    }

    fn take(self) -> Option<T> {
        Rc::into_inner(self.0).map(|cell| cell.into_inner())
    }
}

trait Map {
    type Key;
    type Val;
    fn get(&self, i: Self::Key) -> Option<&Self::Val>;
}

impl<I: Id, V> Map for IdVec<I, V> {
    type Key = I;
    type Val = V;
    fn get(&self, i: I) -> Option<&V> {
        self.get(i)
    }
}

impl<I: Id, V> Map for IdMap<I, V> {
    type Key = I;
    type Val = V;
    fn get(&self, i: I) -> Option<&V> {
        self.get(i)
    }
}

fn count_lt_params<'a, A: 'a, B: 'a, C: 'a, D: 'a>(
    customs: &'a impl Map<Key = CustomTypeId, Val = annot::TypeDef>,
    ty: annot::TypeLike<'a, A, B, C, D>,
) -> usize {
    ty.map(
        |_, id, _| match customs.get(id) {
            Some(typedef) => typedef.num_lt_params.to_value(),
            // This is a typedef in the same SCC; the reference to it here contributes no additional
            // parameters to the entire SCC.
            None => 0,
        },
        |_, _| 1,
        |_, _, _| 0,
        |_, _| 0,
    )
    .fold(0, |a, b| a + b)
}

fn count_mode_params<'a, A: 'a, B: 'a, C: 'a, D: 'a>(
    customs: &'a impl Map<Key = CustomTypeId, Val = annot::TypeDef>,
    ty: annot::TypeLike<'a, A, B, C, D>,
) -> usize {
    ty.map(
        |_, id, _| match customs.get(id) {
            Some(typedef) => typedef.num_mode_params.to_value(),
            // This is a typedef in the same SCC; the reference to it here contributes no additional
            // parameters to the entire SCC.
            None => 0,
        },
        |_, _| 1,
        |_, id, _| match customs.get(id) {
            Some(typedef) => typedef.overlay_params.len(),
            // This is a typedef in the same SCC; the reference to it here contributes no additional
            // parameters to the entire SCC.
            None => 0,
        },
        |_, _| 1,
    )
    .fold(0, |a, b| a + b)
}

// Records the set of overlay parameters the would be generated by `parameterize_overlay` given
// `parameterized`, `next_mode`, and `ty`.
fn accum_overlay_params(
    params: &mut BTreeSet<ModeParam>,
    parameterized: &IdMap<CustomTypeId, annot::TypeDef>,
    next_mode: &mut Count<ModeParam>,
    ty: &anon::Type,
) {
    let next_mode = state(next_mode);
    let params = state(params);
    ty.overlay_items().for_each(
        |id, _| match parameterized.get(id) {
            Some(typedef) => params
                .get()
                .extend(typedef.overlay_params.iter().map(|_| next_mode.get().inc())),
            None => {}
        },
        |_| {
            params.get().insert(next_mode.get().inc());
        },
    );
}

fn parameterize_overlay_custom(
    parameterized: &IdMap<CustomTypeId, annot::TypeDef>,
    scc_overlay_params: &BTreeSet<ModeParam>,
    next_mode: State<&mut Count<ModeParam>>,
    id: CustomTypeId,
) -> BTreeMap<ModeParam, ModeParam> {
    match parameterized.get(id) {
        Some(typedef) => typedef
            .overlay_params
            .iter()
            .map(|id| (*id, next_mode.get().inc()))
            .collect::<BTreeMap<_, _>>(),
        // This is a typedef in the same SCC; we need to parameterize it by all the SCC
        // parameters
        None => scc_overlay_params.iter().map(|id| (*id, *id)).collect(),
    }
}

fn parameterize_overlay(
    parameterized: &IdMap<CustomTypeId, annot::TypeDef>,
    scc_overlay_params: &BTreeSet<ModeParam>,
    next_mode: &mut Count<ModeParam>,
    ty: &anon::Type,
) -> Overlay<ModeParam> {
    let next_mode = state(next_mode);
    ty.overlay_items()
        .map(
            |id, _| {
                parameterize_overlay_custom(
                    parameterized,
                    scc_overlay_params,
                    next_mode.clone(),
                    id,
                )
            },
            |_| next_mode.get().inc(),
        )
        .collect()
}

fn parameterize_type(
    parameterized: &IdMap<CustomTypeId, annot::TypeDef>,
    scc_mode_count: Count<ModeParam>,
    scc_lt_count: Count<LtParam>,
    scc_overlay_params: &BTreeSet<ModeParam>,
    next_mode: &mut Count<ModeParam>,
    next_lt: &mut Count<LtParam>,
    ty: &anon::Type,
) -> annot::Type<ModeParam, LtParam> {
    let next_mode = state(next_mode);
    let next_lt = state(next_lt);
    ty.items()
        .map(
            |_, id, _| {
                match parameterized.get(id) {
                    Some(typedef) => (
                        IdVec::from_count_with(typedef.num_mode_params, |_| next_mode.get().inc()),
                        IdVec::from_count_with(typedef.num_lt_params, |_| next_lt.get().inc()),
                    ),
                    // This is a typedef in the same SCC; we need to parameterize it by all the SCC
                    // parameters
                    None => (
                        IdVec::from_count_with(scc_mode_count, |id| id),
                        IdVec::from_count_with(scc_lt_count, |id| id),
                    ),
                }
            },
            |_, _| (next_mode.get().inc(), next_lt.get().inc()),
            |_, id, _| {
                parameterize_overlay_custom(
                    parameterized,
                    scc_overlay_params,
                    next_mode.clone(),
                    id,
                )
            },
            |_, _| next_mode.get().inc(),
        )
        .collect()
}

fn parameterize_custom_scc(
    customs: &IdVec<CustomTypeId, flat::CustomTypeDef>,
    parameterized: &IdMap<CustomTypeId, annot::TypeDef>,
    scc: &old_graph::Scc<CustomTypeId>,
) -> BTreeMap<CustomTypeId, annot::TypeDef> {
    let num_mode_params = Count::from_value(
        scc.iter()
            .map(|id| count_mode_params(parameterized, customs[*id].content.items()))
            .sum(),
    );

    let num_lt_params = Count::from_value(
        scc.iter()
            .map(|id| count_lt_params(parameterized, customs[*id].content.items()))
            .sum(),
    );

    // Do a "dry run" of `parameterize_overlay` to determine `overlay_params`
    let mut overlay_params = BTreeSet::new();
    let mut next_mode = Count::new();
    for id in scc.iter() {
        accum_overlay_params(
            &mut overlay_params,
            parameterized,
            &mut next_mode,
            &customs[*id].content,
        );
    }

    // Now, reset `next_mode` and actually generate parameterized overlays
    let mut next_mode = Count::new();
    let overlays = scc
        .iter()
        .map(|id| {
            parameterize_overlay(
                parameterized,
                &overlay_params,
                &mut next_mode,
                &customs[*id].content,
            )
        })
        .collect::<Vec<_>>();

    // We need to keep the same mode counter runnning which was used in overlay generation when we
    // generate types below
    let mut next_lt = Count::new();

    let to_populate = scc
        .iter()
        .zip(overlays.into_iter())
        .map(|(id, overlay)| {
            (
                *id,
                annot::TypeDef {
                    num_mode_params,
                    num_lt_params,
                    overlay_params: overlay_params.clone(),
                    content: parameterize_type(
                        parameterized,
                        num_mode_params,
                        num_lt_params,
                        &overlay_params,
                        &mut next_mode,
                        &mut next_lt,
                        &customs[*id].content,
                    ),
                    overlay,
                },
            )
        })
        .collect();

    debug_assert_eq!(num_mode_params, next_mode);
    debug_assert_eq!(num_lt_params, next_lt);
    to_populate
}

fn parameterize_customs(customs: &flat::CustomTypes) -> IdVec<CustomTypeId, annot::TypeDef> {
    let mut parameterized = IdMap::new();

    for scc in customs.sccs.values() {
        let to_populate = parameterize_custom_scc(&customs.types, &parameterized, scc);

        debug_assert_eq!(
            to_populate.keys().collect::<BTreeSet<_>>(),
            scc.iter().collect::<BTreeSet<_>>()
        );

        for (id, typedef) in to_populate {
            parameterized.insert(id, typedef);
        }
    }
    parameterized.to_id_vec(customs.types.count())
}

#[id_type]
struct ModeVar(usize);

type SolverType = annot::Type<ModeVar, Lt>;
type SolverOccur = annot::Occur<ModeVar, Lt>;
type SolverOverlay = annot::Overlay<ModeVar>;
type SolverCondition = annot::Condition<ModeVar, Lt>;
type SolverExpr = annot::Expr<ModeVar, Lt>;

#[id_type]
struct EquivClass(usize);

struct EquivClasses {
    classes: IdVec<ModeVar, EquivClass>,
    reverse: IdVec<EquivClass, ModeVar>,
}

impl EquivClasses {
    fn class(&self, var: ModeVar) -> EquivClass {
        self.classes[var]
    }

    fn count(&self) -> usize {
        self.classes.len()
    }

    fn one_repr_var_of(&self, class: EquivClass) -> ModeVar {
        self.reverse[class]
    }
}

struct Solution {
    classes: EquivClasses,
    modes: IdVec<EquivClass, Mode<ModeVar>>,
    edges_out: BTreeMap<EquivClass, Vec<EquivClass>>,
}

impl Solution {
    fn iter_lte_constrs<'a>(&'a self) -> impl Iterator<Item = (EquivClass, EquivClass)> + 'a {
        self.edges_out
            .iter()
            .flat_map(|(&b, neighbors)| neighbors.iter().copied().zip(iter::repeat(b)))
    }
}

struct ConstrGraph {
    // b -> a means a <= b, i.e. if a is owned then b is owned; both `ConstrGraph` and `Solution`
    // rely on this
    edges_out: IdVec<ModeVar, Vec<ModeVar>>,
    owned: BTreeSet<ModeVar>,
}

impl ConstrGraph {
    fn new() -> Self {
        Self {
            edges_out: IdVec::new(),
            owned: BTreeSet::new(),
        }
    }

    fn fresh_var(&mut self) -> ModeVar {
        self.edges_out.push(Vec::new())
    }

    fn force_lte(&mut self, a: ModeVar, b: ModeVar) {
        self.edges_out[b].push(a);
    }

    fn force_eq(&mut self, a: ModeVar, b: ModeVar) {
        self.force_lte(a, b);
        self.force_lte(b, a);
    }

    fn force_owned(&mut self, a: ModeVar) {
        self.owned.insert(a);
    }

    fn solve_for(&self, sig_vars: &BTreeSet<ModeVar>) -> Solution {
        let sccs: Sccs<EquivClass, _> =
            find_components(self.edges_out.count(), |var| &self.edges_out[var]);

        let mut classes = IdMap::new();
        let mut reverse = IdMap::new();

        // Initialize `classes` and `reverse`, ensuring that the representative of any class
        // that contains a signature variable is a signature variable
        for (class, scc) in &sccs {
            for &var in scc.nodes {
                classes.insert_vacant(var, class);
                if !reverse.contains_key(class) && sig_vars.contains(&var) {
                    reverse.insert_vacant(class, var);
                }
            }
            if !reverse.contains_key(class) {
                reverse.insert_vacant(class, scc.nodes[0]);
            }
        }

        let classes = classes.to_id_vec(self.edges_out.count());
        let reverse = reverse.to_id_vec(sccs.count());

        // Start by setting all classes which are trivially owned to owned
        let mut modes = IdMap::new();
        for var in &self.owned {
            modes.insert_vacant(classes[var], Mode::Owned);
        }

        // Now, propagate owned-ness to all classes which are transitively reachable from an owned
        // class; this relies on the fact that `classes` is topologically sorted
        for (var1, class) in &classes {
            if modes.contains_key(class) {
                for var2 in &self.edges_out[var1] {
                    modes.insert_vacant(classes[var2], Mode::Owned);
                }
            }
        }

        let mut edges_out = BTreeMap::new();
        let mut external = sig_vars
            .iter()
            .map(|var| classes[var])
            .collect::<BTreeSet<_>>();

        // Finally, fill in the remaining modes
        for class in sccs.count().into_iter() {
            if !modes.contains_key(class) {
                if external.contains(&class) {
                    // This class is "external" (dependent on a signature variable). We won't be
                    // able to find a concrete solution for it until we instantiate the relevant
                    // function signature.

                    // Pick a representative variable for the class
                    modes.insert_vacant(class, Mode::Param(reverse[class]));

                    let class_edges_out = sccs
                        .component(class)
                        .nodes
                        .iter()
                        .flat_map(|var| &self.edges_out[var])
                        .map(|var| classes[var])
                        .collect::<BTreeSet<_>>();

                    // Mark all the classes which depend on this class as "external"
                    for &other in &class_edges_out {
                        external.insert(other);
                    }

                    // Record all the classes which depend on this class in `edges_out`
                    let old = edges_out.insert(class, class_edges_out.into_iter().collect());
                    debug_assert!(old.is_none());
                }
            }
        }

        let modes = modes.to_id_vec(sccs.count());

        Solution {
            classes: EquivClasses { classes, reverse },
            modes,
            edges_out,
        }
    }
}

// Returns the unifier for the lifetimes of `ty1` and `ty2`.
// TODO: `ty1` should really be a `annot::Type<ModeVar, LtParam>`
fn lt_bind(ty1: &SolverType, ty2: &SolverType) -> BTreeMap<LtParam, Lt> {
    fn extract_param(lt: &Lt) -> LtParam {
        match lt {
            Lt::Join(params) if params.len() == 1 => *params.first().unwrap(),
            _ => panic!("expected a parameter"),
        }
    }

    fn lt_bind_rec(ty1: &SolverType, ty2: &SolverType, subst: &mut BTreeMap<LtParam, Lt>) {
        match (ty1, ty2) {
            (annot::Type::Bool, annot::Type::Bool) => {}
            (annot::Type::Num(n1), annot::Type::Num(n2)) if n1 == n2 => {}
            (annot::Type::Tuple(tys1), annot::Type::Tuple(tys2)) => {
                for (ty1, ty2) in tys1.iter().zip_eq(tys2) {
                    lt_bind_rec(ty1, ty2, subst);
                }
            }
            (annot::Type::Variants(tys1), annot::Type::Variants(tys2)) => {
                for ((_, ty1), (_, ty2)) in tys1.iter().zip_eq(tys2) {
                    lt_bind_rec(ty1, ty2, subst);
                }
            }
            (annot::Type::Custom(id1, _, lts1), annot::Type::Custom(id2, _, lts2))
                if id1 == id2 =>
            {
                for (lt1, lt2) in lts1.values().zip_eq(lts2.values()) {
                    subst.insert_vacant(extract_param(lt1), lt2.clone());
                }
            }
            (annot::Type::Array(_, lt1, ty1, _), annot::Type::Array(_, lt2, ty2, _))
            | (annot::Type::HoleArray(_, lt1, ty1, _), annot::Type::HoleArray(_, lt2, ty2, _))
            | (annot::Type::Boxed(_, lt1, ty1, _), annot::Type::Boxed(_, lt2, ty2, _)) => {
                subst.insert_vacant(extract_param(lt1), lt2.clone());
                lt_bind_rec(ty1, ty2, subst)
            }
            _ => panic!("incompatible types"),
        }
    }

    let mut subst = BTreeMap::new();
    lt_bind_rec(ty1, ty2, &mut subst);
    subst
}

fn mode_bind(constrs: &mut ConstrGraph, ty1: &SolverType, ty2: &SolverType) {
    let constrs = state(constrs);
    ty1.items().zip(ty2.items()).for_each(
        |_, _, ((ms1, _), (ms2, _))| {
            for (m1, m2) in ms1.values().zip_eq(ms2.values()) {
                constrs.get().force_eq(*m1, *m2);
            }
        },
        |_, ((m1, _), (m2, _))| {
            constrs.get().force_eq(*m1, *m2);
        },
        |_, _, (ms1, ms2)| {
            for (m1, m2) in ms1.values().zip_eq(ms2.values()) {
                constrs.get().force_eq(*m1, *m2);
            }
        },
        |_, (m1, m2)| {
            constrs.get().force_eq(*m1, *m2);
        },
    )
}

fn emit_occur_constr(
    constrs: &mut ConstrGraph,
    path: &annot::Path,
    binding_lt: &Lt,
    use_lt: &Lt,
    binding_mode: ModeVar,
    use_mode: ModeVar,
) {
    if use_lt.does_not_exceed(path) {
        if *binding_lt == Lt::Empty && *use_lt != Lt::Empty {
            // Case: this is a non-escaping, final ("opportunistic") occurrence.
            constrs.force_lte(use_mode, binding_mode);
        }
    } else {
        // Case: this is an escaping ("move" or "dup") occurrence.
        constrs.force_lte(binding_mode, use_mode);
    }
}

fn apply_subst_overlay(
    overlay: &annot::Overlay<ModeParam>,
    subst: &BTreeMap<ModeParam, ModeVar>,
) -> SolverOverlay {
    overlay
        .items()
        .map(
            |_, ms| ms.iter().map(|(p, m)| (*p, subst[m])).collect(),
            |m| subst[m],
        )
        .collect()
}

fn apply_subst_type(
    ty: &annot::Type<ModeParam, LtParam>,
    mode_subst: &IdVec<ModeParam, ModeVar>,
    lt_subst: &IdVec<LtParam, Lt>,
) -> SolverType {
    ty.items()
        .map(
            |_, _, (ms, lts)| {
                (
                    ms.map_refs(|_, m| mode_subst[m]),
                    lts.map_refs(|_, lt| lt_subst[lt].clone()),
                )
            },
            |_, (m, lt)| (mode_subst[m], lt_subst[lt].clone()),
            |_, _, ms| ms.iter().map(|(p, m)| (*p, mode_subst[m])).collect(),
            |_, m| mode_subst[m],
        )
        .collect()
}

fn emit_occur_constrs_overlay(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    constrs: &mut ConstrGraph,
    path: &annot::Path,
    binding_ty: &SolverType,
    fut_ty: &SolverType,
    binding_ov: &SolverOverlay,
    fut_ov: &SolverOverlay,
) {
    match (binding_ty, fut_ty, binding_ov, fut_ov) {
        (annot::Type::Bool, annot::Type::Bool, annot::Overlay::Bool, annot::Overlay::Bool) => {}
        (
            annot::Type::Num(n1),
            annot::Type::Num(n2),
            annot::Overlay::Num(n3),
            annot::Overlay::Num(n4),
        ) if n2 == n1 && n3 == n1 && n4 == n1 => {}
        (
            annot::Type::Tuple(tys1),
            annot::Type::Tuple(tys2),
            annot::Overlay::Tuple(os1),
            annot::Overlay::Tuple(os2),
        ) => {
            for (((ty1, ty2), o1), o2) in tys1.iter().zip_eq(tys2).zip_eq(os1).zip_eq(os2) {
                emit_occur_constrs_overlay(customs, constrs, path, ty1, ty2, o1, o2);
            }
        }
        (
            annot::Type::Variants(tys1),
            annot::Type::Variants(tys2),
            annot::Overlay::Variants(os1),
            annot::Overlay::Variants(os2),
        ) => {
            for (((ty1, ty2), o1), o2) in tys1
                .values()
                .zip_eq(tys2.values())
                .zip_eq(os1.values())
                .zip_eq(os2.values())
            {
                emit_occur_constrs_overlay(customs, constrs, path, ty1, ty2, o1, o2);
            }
        }
        (
            annot::Type::Custom(id1, ms1, lts1),
            annot::Type::Custom(id2, ms2, lts2),
            annot::Overlay::Custom(id3, os1),
            annot::Overlay::Custom(id4, os2),
        ) if id2 == id1 && id3 == id1 && id4 == id1 => {
            let typedef = &customs[id1];
            let ty1 = apply_subst_type(&typedef.content, ms1, lts1);
            let ty2 = apply_subst_type(&typedef.content, ms2, lts2);
            let ov1 = apply_subst_overlay(&typedef.overlay, os1);
            let ov2 = apply_subst_overlay(&typedef.overlay, os2);
            emit_occur_constrs_overlay(customs, constrs, path, &ty1, &ty2, &ov1, &ov2);
        }
        (
            annot::Type::Array(_, lt1, _, _),
            annot::Type::Array(_, lt2, _, _),
            annot::Overlay::Array(m1),
            annot::Overlay::Array(m2),
        )
        | (
            annot::Type::HoleArray(_, lt1, _, _),
            annot::Type::HoleArray(_, lt2, _, _),
            annot::Overlay::HoleArray(m1),
            annot::Overlay::HoleArray(m2),
        )
        | (
            annot::Type::Boxed(_, lt1, _, _),
            annot::Type::Boxed(_, lt2, _, _),
            annot::Overlay::Boxed(m1),
            annot::Overlay::Boxed(m2),
        ) => {
            emit_occur_constr(constrs, path, lt1, lt2, *m1, *m2);
        }
        _ => panic!("incompatible types"),
    }
}

fn emit_occur_constrs_heap(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    constrs: &mut ConstrGraph,
    path: &annot::Path,
    binding_ty: &SolverType,
    fut_ty: &SolverType,
) {
    match (binding_ty, fut_ty) {
        (annot::Type::Bool, annot::Type::Bool) => {}
        (annot::Type::Num(n1), annot::Type::Num(n2)) if n1 == n2 => {}
        (annot::Type::Tuple(tys1), annot::Type::Tuple(tys2)) => {
            for (ty1, ty2) in tys1.iter().zip_eq(tys2) {
                emit_occur_constrs_heap(customs, constrs, path, ty1, ty2);
            }
        }
        (annot::Type::Variants(tys1), annot::Type::Variants(tys2)) => {
            for ((_, ty1), (_, ty2)) in tys1.iter().zip_eq(tys2) {
                emit_occur_constrs_heap(customs, constrs, path, ty1, ty2);
            }
        }
        (annot::Type::Custom(id1, ms1, lts1), annot::Type::Custom(id2, ms2, lts2))
            if id1 == id2 =>
        {
            let lhs = ms1.values().zip_eq(lts1.values());
            let rhs = ms2.values().zip_eq(lts2.values());
            for ((m1, lt1), (m2, lt2)) in lhs.zip_eq(rhs) {
                emit_occur_constr(constrs, path, lt1, lt2, *m1, *m2);
            }
        }
        (annot::Type::Array(m1, _, ty1, o1), annot::Type::Array(m2, _, ty2, o2))
        | (annot::Type::HoleArray(m1, _, ty1, o1), annot::Type::HoleArray(m2, _, ty2, o2))
        | (annot::Type::Boxed(m1, _, ty1, o1), annot::Type::Boxed(m2, _, ty2, o2)) => {
            constrs.force_eq(*m1, *m2);
            emit_occur_constrs_heap(customs, constrs, path, ty1, ty2);
            emit_occur_constrs_overlay(customs, constrs, path, ty1, ty2, o1, o2);
        }
        _ => panic!("incompatible types"),
    }
}

fn emit_occur_constrs(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    constrs: &mut ConstrGraph,
    path: &annot::Path,
    binding_ty: &SolverType,
    fut_ty: &SolverType,
) {
    match (binding_ty, fut_ty) {
        (annot::Type::Bool, annot::Type::Bool) => {}
        (annot::Type::Num(n1), annot::Type::Num(n2)) if n1 == n2 => {}
        (annot::Type::Tuple(tys1), annot::Type::Tuple(tys2)) => {
            for (ty1, ty2) in tys1.iter().zip_eq(tys2) {
                emit_occur_constrs(customs, constrs, path, ty1, ty2);
            }
        }
        (annot::Type::Variants(tys1), annot::Type::Variants(tys2)) => {
            for ((_, ty1), (_, ty2)) in tys1.iter().zip_eq(tys2) {
                emit_occur_constrs(customs, constrs, path, ty1, ty2);
            }
        }
        (annot::Type::Custom(id1, ms1, lts1), annot::Type::Custom(id2, ms2, lts2))
            if id1 == id2 =>
        {
            // if not self
            //   ty1 = instantiate_type typedef.content ms1 lts1
            //   ty2 = instantiate_type typedef.content ms2 lts2
            //   ov1 = instantiate_overlay typedef.overlay ms1
            //   ov2 = instantiate_overlay typedef.overlay ms2
            //   emit_occur_constrs_overlay customs constrs path ty1 ty2 ov1 ov2
            let lhs = ms1.values().zip_eq(lts1.values());
            let rhs = ms2.values().zip_eq(lts2.values());
            for ((m1, lt1), (m2, lt2)) in lhs.zip_eq(rhs) {
                emit_occur_constr(constrs, path, lt1, lt2, *m1, *m2);
            }
        }
        (annot::Type::Array(m1, lt1, ty1, o1), annot::Type::Array(m2, lt2, ty2, o2))
        | (annot::Type::HoleArray(m1, lt1, ty1, o1), annot::Type::HoleArray(m2, lt2, ty2, o2))
        | (annot::Type::Boxed(m1, lt1, ty1, o1), annot::Type::Boxed(m2, lt2, ty2, o2)) => {
            emit_occur_constr(constrs, path, lt1, lt2, *m1, *m2);
            emit_occur_constrs_heap(customs, constrs, path, ty1, ty2);
            emit_occur_constrs_overlay(customs, constrs, path, ty1, ty2, o1, o2);
        }
        _ => panic!("incompatible types"),
    }
}

#[derive(Clone, Debug)]
struct ImmutContext {
    count: Count<LocalId>,
    stack: im_rc::Vector<Rc<SolverType>>, // TODO: `Vector` is slow
}

impl ImmutContext {
    fn new() -> Self {
        Self {
            count: Count::new(),
            stack: im_rc::Vector::new(),
        }
    }

    fn add_local(&mut self, binding: Rc<SolverType>) -> LocalId {
        let id = self.count.inc();
        self.stack.push_back(binding);
        id
    }

    fn set_local(&mut self, local: LocalId, binding: Rc<SolverType>) {
        self.stack[local.0] = binding;
    }

    fn local_binding(&self, local: LocalId) -> &Rc<SolverType> {
        &self.stack[local.0]
    }
}

#[derive(Clone, Debug)]
struct LocalUpdates(BTreeMap<LocalId, Rc<SolverType>>);

impl LocalUpdates {
    fn new() -> Self {
        LocalUpdates(BTreeMap::new())
    }

    fn local_binding(&self, local: LocalId) -> Option<&Rc<SolverType>> {
        self.0.get(&local)
    }

    fn record(&mut self, id: LocalId, binding: Rc<SolverType>) {
        self.0.insert(id, binding);
    }

    fn merge(&mut self, other: &Self) {
        use std::collections::btree_map::Entry;
        for (id, ty) in &other.0 {
            match self.0.entry(*id) {
                Entry::Vacant(entry) => {
                    entry.insert(ty.clone());
                }
                Entry::Occupied(mut entry) => {
                    let old = entry.get_mut();
                    *old = Rc::new(old.left_meet(&ty));
                }
            }
        }
    }
}

#[derive(Clone, Debug)]
struct TrackedContext {
    ctx: ImmutContext,
    updates: LocalUpdates,
}

impl TrackedContext {
    fn new(ctx: &ImmutContext) -> Self {
        Self {
            ctx: ctx.clone(),
            updates: LocalUpdates::new(),
        }
    }

    fn add_local(&mut self, binding: Rc<SolverType>) -> LocalId {
        let local = self.ctx.add_local(binding.clone());
        self.updates.record(local, binding);
        local
    }

    fn set_local(&mut self, local: LocalId, binding: Rc<SolverType>) {
        self.ctx.set_local(local, binding.clone());
        self.updates.record(local, binding);
    }

    fn set_locals(&mut self, updates: &LocalUpdates) {
        for (id, binding) in &updates.0 {
            self.ctx.set_local(*id, binding.clone());
        }
    }

    fn local_binding(&self, local: LocalId) -> &Rc<SolverType> {
        self.ctx.local_binding(local)
    }

    fn as_untracked(&self) -> &ImmutContext {
        &self.ctx
    }

    fn into_updates(self) -> LocalUpdates {
        self.updates
    }
}

fn same_shape(ty1: &anon::Type, ty2: &SolverType) -> bool {
    match (ty1, ty2) {
        (anon::Type::Bool, annot::Type::Bool) => true,
        (anon::Type::Num(num_ty1), annot::Type::Num(num_ty2)) => num_ty1 == num_ty2,
        (anon::Type::Tuple(tys1), annot::Type::Tuple(tys2)) => {
            tys1.len() == tys2.len()
                && tys1
                    .iter()
                    .zip(tys2.iter())
                    .all(|(ty1, ty2)| same_shape(ty1, ty2))
        }
        (anon::Type::Variants(tys1), annot::Type::Variants(tys2)) => {
            tys1.len() == tys2.len()
                && tys1
                    .iter()
                    .zip(tys2.iter())
                    .all(|((_, ty1), (_, ty2))| same_shape(ty1, ty2))
        }
        (anon::Type::Custom(id1), annot::Type::Custom(id2, _, _)) => id1 == id2,
        (anon::Type::Array(ty1), annot::Type::Array(_, _, ty2, _)) => same_shape(ty1, ty2),
        (anon::Type::HoleArray(ty1), annot::Type::HoleArray(_, _, ty2, _)) => same_shape(ty1, ty2),
        (anon::Type::Boxed(ty1), annot::Type::Boxed(_, _, ty2, _)) => same_shape(ty1, ty2),
        _ => false,
    }
}

fn apply_overlay<L: Clone>(
    ty: &annot::Type<ModeVar, L>,
    overlay: &SolverOverlay,
) -> annot::Type<ModeVar, L> {
    match (ty, overlay) {
        (annot::Type::Bool, annot::Overlay::Bool) => annot::Type::Bool,
        (annot::Type::Num(num_ty1), annot::Overlay::Num(num_ty2)) => {
            assert_eq!(num_ty1, num_ty2);
            annot::Type::Num(*num_ty1)
        }
        (annot::Type::Tuple(tys), annot::Overlay::Tuple(os)) => {
            assert_eq!(tys.len(), os.len());
            let tys = tys
                .iter()
                .zip(os.iter())
                .map(|(ty, o)| apply_overlay(ty, o))
                .collect();
            annot::Type::Tuple(tys)
        }
        (annot::Type::Variants(tys), annot::Overlay::Variants(os)) => {
            assert_eq!(tys.len(), os.len());
            let tys = tys
                .values()
                .zip(os.values())
                .map(|(ty, o)| apply_overlay(ty, o))
                .collect();
            annot::Type::Variants(IdVec::from_vec(tys))
        }
        (annot::Type::Custom(id1, ms1, lts), annot::Overlay::Custom(id2, ms2)) => {
            assert_eq!(id1, id2);
            annot::Type::Custom(
                *id1,
                // TODO: make `Subst` and `OverlaySubst` types
                ms1.map_refs(|id, var| *ms2.get(&id).unwrap_or(var)),
                lts.clone(),
            )
        }
        (annot::Type::Array(_, lt, ty, o), annot::Overlay::Array(m)) => {
            annot::Type::Array(*m, lt.clone(), ty.clone(), o.clone())
        }
        (annot::Type::HoleArray(_, lt, ty, o), annot::Overlay::HoleArray(m)) => {
            annot::Type::HoleArray(*m, lt.clone(), ty.clone(), o.clone())
        }
        (annot::Type::Boxed(_, lt, ty, o), annot::Overlay::Boxed(m)) => {
            annot::Type::Boxed(*m, lt.clone(), ty.clone(), o.clone())
        }
        _ => panic!("type and overlay are incompatible"),
    }
}

fn obtain_overlay<L>(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    ty: &annot::Type<ModeVar, L>,
) -> SolverOverlay {
    match ty {
        annot::Type::Bool => annot::Overlay::Bool,
        annot::Type::Num(num_ty) => annot::Overlay::Num(*num_ty),
        annot::Type::Tuple(tys) => {
            annot::Overlay::Tuple(tys.iter().map(|ty| obtain_overlay(customs, ty)).collect())
        }
        annot::Type::Variants(tys) => {
            annot::Overlay::Variants(tys.map_refs(|_, ty| obtain_overlay(customs, ty)))
        }
        annot::Type::Custom(id, ms, _) => annot::Overlay::Custom(
            *id,
            // TODO: make `Subst` and `OverlaySubst` types
            customs[*id]
                .overlay_params
                .iter()
                .map(|id| (*id, *ms.get(id).unwrap()))
                .collect(),
        ),
        annot::Type::Array(m, _, _, _) => annot::Overlay::Array(*m),
        annot::Type::HoleArray(m, _, _, _) => annot::Overlay::HoleArray(*m),
        annot::Type::Boxed(m, _, _, _) => annot::Overlay::Boxed(*m),
    }
}

fn unfold_custom_type(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    sccs: &IdVec<CustomTypeId, flat::CustomTypeSccId>,
    ty: &SolverType,
) -> SolverType {
    fn unfold_overlay(
        sccs: &IdVec<CustomTypeId, flat::CustomTypeSccId>,
        scc_id: CustomTypeSccId,
        overlay: &annot::Overlay<ModeParam>,
        mode_subst: &IdVec<ModeParam, ModeVar>,
        overlay_subst: &BTreeMap<ModeParam, ModeVar>,
    ) -> SolverOverlay {
        match overlay {
            annot::Overlay::Bool => annot::Overlay::Bool,
            annot::Overlay::Num(num_ty) => annot::Overlay::Num(*num_ty),
            annot::Overlay::Tuple(os) => annot::Overlay::Tuple(
                os.iter()
                    .map(|o| unfold_overlay(sccs, scc_id, o, mode_subst, overlay_subst))
                    .collect(),
            ),
            annot::Overlay::Variants(os) => annot::Overlay::Variants(
                os.map_refs(|_, o| unfold_overlay(sccs, scc_id, o, mode_subst, overlay_subst)),
            ),
            annot::Overlay::Custom(id, os) => annot::Overlay::Custom(
                *id,
                if scc_id == sccs[*id] {
                    overlay_subst.clone()
                } else {
                    assert_eq!(os.len(), mode_subst.len());
                    os.iter().map(|(p, m)| (*p, mode_subst[m])).collect()
                },
            ),
            annot::Overlay::Array(m) => annot::Overlay::Array(mode_subst[m]),
            annot::Overlay::HoleArray(m) => annot::Overlay::HoleArray(mode_subst[m]),
            annot::Overlay::Boxed(m) => annot::Overlay::Boxed(mode_subst[m]),
        }
    }

    fn unfold_type(
        customs: &IdVec<CustomTypeId, annot::TypeDef>,
        sccs: &IdVec<CustomTypeId, flat::CustomTypeSccId>,
        scc_id: CustomTypeSccId,
        extracted: &BTreeMap<ModeParam, ModeVar>,
        ty: &annot::Type<ModeParam, LtParam>,
        mode_subst: &IdVec<ModeParam, ModeVar>,
        lt_subst: &IdVec<LtParam, Lt>,
        overlay_subst: &BTreeMap<ModeParam, ModeVar>,
    ) -> SolverType {
        match ty {
            annot::Type::Bool => annot::Type::Bool,
            annot::Type::Num(num_ty) => annot::Type::Num(*num_ty),
            annot::Type::Tuple(tys) => annot::Type::Tuple(
                tys.iter()
                    .map(|ty| {
                        unfold_type(
                            customs,
                            sccs,
                            scc_id,
                            extracted,
                            ty,
                            mode_subst,
                            lt_subst,
                            overlay_subst,
                        )
                    })
                    .collect(),
            ),
            annot::Type::Variants(tys) => annot::Type::Variants(tys.map_refs(|_, ty| {
                unfold_type(
                    customs,
                    sccs,
                    scc_id,
                    extracted,
                    ty,
                    mode_subst,
                    lt_subst,
                    overlay_subst,
                )
            })),
            annot::Type::Custom(id, ms, lts) => annot::Type::Custom(
                *id,
                ms.map_refs(|_, m| mode_subst[m]),
                lts.map_refs(|_, lt| lt_subst[*lt].clone()),
            ),
            annot::Type::Array(m, lt, ty, o) => annot::Type::Array(
                mode_subst[m],
                lt_subst[*lt].clone(),
                Box::new(unfold_type(
                    customs,
                    sccs,
                    scc_id,
                    extracted,
                    ty,
                    mode_subst,
                    lt_subst,
                    overlay_subst,
                )),
                unfold_overlay(sccs, scc_id, o, mode_subst, overlay_subst),
            ),
            annot::Type::HoleArray(m, lt, ty, o) => annot::Type::HoleArray(
                mode_subst[m],
                lt_subst[*lt].clone(),
                Box::new(unfold_type(
                    customs,
                    sccs,
                    scc_id,
                    extracted,
                    ty,
                    mode_subst,
                    lt_subst,
                    overlay_subst,
                )),
                unfold_overlay(sccs, scc_id, o, mode_subst, overlay_subst),
            ),
            annot::Type::Boxed(m, lt, ty, o) => annot::Type::Boxed(
                mode_subst[m],
                lt_subst[*lt].clone(),
                Box::new(unfold_type(
                    customs,
                    sccs,
                    scc_id,
                    extracted,
                    ty,
                    mode_subst,
                    lt_subst,
                    overlay_subst,
                )),
                unfold_overlay(sccs, scc_id, o, mode_subst, overlay_subst),
            ),
        }
    }

    let annot::Type::Custom(id, ms, lts) = ty else {
        panic!("expected custom type");
    };
    let typedef = &customs[*id];
    let ty = apply_subst_type(&typedef.content, ms, lts);

    let mut extracted = BTreeMap::new();
    let extracted_st = state(&mut extracted);
    typedef.overlay.items().zip(ty.overlay_items()).for_each(
        |_, (params, ms)| {
            for (param, m) in params.values().zip(ms.values()) {
                extracted_st.get().insert_vacant(*param, *m);
            }
        },
        |(param, m)| {
            extracted_st.get().insert_vacant(*param, *m);
        },
    );

    let overlay_subst = ms
        .iter()
        .filter(|(p, _)| typedef.overlay_params.contains(p))
        .map(|(p, m)| (p, *m))
        .collect();
    unfold_type(
        customs,
        sccs,
        sccs[*id],
        &extracted,
        &typedef.content,
        &ms,
        lts,
        &overlay_subst,
    )
}

fn instantiate_occur(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    ctx: &mut TrackedContext,
    constrs: &mut ConstrGraph,
    path: &annot::Path,
    id: LocalId,
    fut_ty: &SolverType,
) -> SolverOccur {
    let old_ty = ctx.local_binding(id);
    emit_occur_constrs(customs, constrs, path, old_ty, fut_ty);

    let new_ty = Rc::new(old_ty.left_meet(fut_ty));
    ctx.set_local(id, new_ty);

    annot::Occur {
        local: id,
        ty: fut_ty.clone(),
    }
}

fn instantiate_condition(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    constrs: &mut ConstrGraph,
    cond: &flat::Condition,
) -> SolverCondition {
    match cond {
        flat::Condition::Any => annot::Condition::Any,
        flat::Condition::Tuple(conds) => annot::Condition::Tuple(
            conds
                .iter()
                .map(|cond| instantiate_condition(customs, constrs, cond))
                .collect(),
        ),
        flat::Condition::Variant(id, cond) => {
            annot::Condition::Variant(*id, Box::new(instantiate_condition(customs, constrs, cond)))
        }
        flat::Condition::Boxed(cond, ty) => annot::Condition::Boxed(
            Box::new(instantiate_condition(customs, constrs, cond)),
            instantiate_type_unused(customs, constrs, ty),
        ),
        flat::Condition::Custom(id, cond) => {
            annot::Condition::Custom(*id, Box::new(instantiate_condition(customs, constrs, cond)))
        }
        flat::Condition::BoolConst(v) => annot::Condition::BoolConst(*v),
        flat::Condition::ByteConst(v) => annot::Condition::ByteConst(*v),
        flat::Condition::IntConst(v) => annot::Condition::IntConst(*v),
        flat::Condition::FloatConst(v) => annot::Condition::FloatConst(*v),
    }
}

fn instantiate_occur_int(ctx: &TrackedContext, id: LocalId) -> SolverOccur {
    assert!(matches!(
        **ctx.local_binding(id),
        annot::Type::Num(NumType::Int)
    ));
    annot::Occur {
        local: id,
        ty: annot::Type::Num(NumType::Int),
    }
}

fn instantiate_occur_array(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    ctx: &mut TrackedContext,
    constrs: &mut ConstrGraph,
    path: &annot::Path,
    item_ty: &anon::Type,
    id: LocalId,
) -> SolverOccur {
    let occur_ty = annot::Type::Array(
        constrs.fresh_var(),
        path.as_lt(),
        Box::new(instantiate_type_unused(customs, constrs, item_ty)),
        instantiate_overlay(customs, constrs, item_ty),
    );
    instantiate_occur(customs, ctx, constrs, path, id, &occur_ty)
}

fn instantiate_occur_hole(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    ctx: &mut TrackedContext,
    constrs: &mut ConstrGraph,
    path: &annot::Path,
    item_ty: &anon::Type,
    id: LocalId,
) -> SolverOccur {
    let occur_ty = annot::Type::Array(
        constrs.fresh_var(),
        path.as_lt(),
        Box::new(instantiate_type_unused(customs, constrs, item_ty)),
        instantiate_overlay(customs, constrs, item_ty),
    );
    instantiate_occur(customs, ctx, constrs, path, id, &occur_ty)
}

// TODO: merge this with `apply_subst_type`
fn apply_subst_lifetimes(ty: &SolverType, subst: &BTreeMap<LtParam, Lt>) -> SolverType {
    let do_subst = |lt: &Lt| match lt {
        Lt::Empty => Lt::Empty,
        Lt::Local(lt) => Lt::Local(lt.clone()),
        Lt::Join(params) => {
            if params.len() == 1 && subst.contains_key(params.first().unwrap()) {
                subst[params.first().unwrap()].clone()
            } else {
                Lt::Join(params.clone())
            }
        }
    };
    ty.items()
        .map(
            |_, _, (ms, lts)| (ms.clone(), lts.map_refs(|_, lt| do_subst(lt))),
            |_, (m, lt)| (*m, do_subst(lt)),
            |_, _, ms| ms.clone(),
            |_, m| *m,
        )
        .collect()
}

fn join_everywhere(ty: &SolverType, lt_new: &Lt) -> SolverType {
    ty.items()
        .map(
            |_, _, (ms, lts)| (ms.clone(), lts.map_refs(|_, lt| lt.join(lt_new))),
            |_, (m, lt)| (*m, lt.join(lt_new)),
            |_, _, ms| ms.clone(),
            |_, m| *m,
        )
        .collect()
}

// This function is the core logic for this pass. It implements the judgement from the paper:
// Δ ; Γ ; S ; q ⊢ e : t ⇝ e ; Q ; Γ'
//
// Note that we must return a set of updates rather than mutating Γ because I-Match requires that we
// check all branches in the initial Γ.
fn instantiate_expr(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    sigs: SignatureAssumptions,
    constrs: &mut ConstrGraph,
    ctx: &ImmutContext,
    scopes: &mut LocalContext<LocalId, annot::Path>,
    path: annot::Path,
    fut_ty: &SolverType,
    expr: &flat::Expr,
) -> (SolverExpr, LocalUpdates) {
    let mut ctx = TrackedContext::new(ctx);

    let expr_annot = match expr {
        flat::Expr::Local(local) => {
            let occur = instantiate_occur(customs, &mut ctx, constrs, &path, *local, fut_ty);
            annot::Expr::Local(occur)
        }

        flat::Expr::Call(purity, func, arg) => {
            let (arg_ty, ret_ty) = sigs.sig_of(constrs, *func);

            mode_bind(constrs, &ret_ty, fut_ty);

            let lt_subst = lt_bind(&ret_ty, fut_ty);
            let arg_ty = join_everywhere(
                &apply_subst_lifetimes(&arg_ty, &lt_subst),
                &path.seq(1).as_lt(),
            );

            let arg = instantiate_occur(customs, &mut ctx, constrs, &path, *arg, &arg_ty);
            annot::Expr::Call(*purity, *func, arg)
        }

        flat::Expr::Branch(discrim_id, cases, ret_ty) => {
            debug_assert!(same_shape(ret_ty, fut_ty));

            let mut updates = LocalUpdates::new();
            let mut cases_annot = Vec::new();
            for (i, (cond, body)) in cases.iter().enumerate() {
                let branch_path = path.par(i, cases.len());
                let cond_annot = instantiate_condition(customs, constrs, cond);
                let (body_annot, body_updates) = instantiate_expr(
                    customs,
                    sigs,
                    constrs,
                    ctx.as_untracked(),
                    scopes,
                    branch_path,
                    fut_ty,
                    body,
                );

                // Record the updates, but DO NOT apply them to the context. Every branch should be
                // checked in the original context.
                updates.merge(&body_updates);

                cases_annot.push((cond_annot, body_annot));
            }

            // Finally, apply the updates before instantiating the discriminant.
            ctx.set_locals(&updates);
            let discrim_occur = instantiate_occur(
                customs,
                &mut ctx,
                constrs,
                &path.seq(0),
                *discrim_id,
                fut_ty,
            );

            annot::Expr::Branch(discrim_occur, cases_annot, fut_ty.clone())
        }

        flat::Expr::LetMany(bindings, result_id) => scopes.with_scope(|scopes| {
            let end_of_scope = path.seq(bindings.len());

            let result_occur = instantiate_occur(
                customs,
                &mut ctx,
                constrs,
                &end_of_scope,
                *result_id,
                fut_ty,
            );

            let mut locals = Vec::new();
            for (binding_ty, _) in bindings {
                let id1 = scopes.add_local(end_of_scope.clone());
                let id2 = if id1 == *result_id {
                    ctx.add_local(Rc::new(instantiate_type_unused(
                        customs, constrs, binding_ty,
                    )))
                } else {
                    ctx.add_local(Rc::new(fut_ty.clone()))
                };
                assert_eq!(id1, id2);
                locals.push(id1);
            }

            let mut exprs_annot = Vec::new();
            for (i, (_, binding_expr)) in bindings.iter().enumerate().rev() {
                let (expr_annot, expr_updates) = instantiate_expr(
                    customs,
                    sigs,
                    constrs,
                    ctx.as_untracked(),
                    scopes,
                    path.seq(i),
                    fut_ty,
                    binding_expr,
                );

                // Record the updates and apply them to the context. It is important that the
                // enclosing loop is in reverse order so that each binding is checking in the
                // correct context.
                ctx.set_locals(&expr_updates);

                exprs_annot.push(expr_annot);
            }

            let new_bindings = locals
                .into_iter()
                .zip(exprs_annot.into_iter())
                .map(|(local, expr_annot)| ((**ctx.local_binding(local)).clone(), expr_annot))
                .collect();

            annot::Expr::LetMany(new_bindings, result_occur)
        }),

        flat::Expr::Tuple(item_ids) => {
            let annot::Type::Tuple(fut_tys) = fut_ty else {
                unreachable!();
            };
            assert_eq!(item_ids.len(), fut_tys.len());
            let occurs = item_ids
                .iter()
                .zip(fut_tys.iter())
                .enumerate()
                .rev()
                .map(|(i, (item_id, item_fut_ty))| {
                    instantiate_occur(
                        customs,
                        &mut ctx,
                        constrs,
                        &path.seq(i),
                        *item_id,
                        item_fut_ty,
                    )
                })
                .collect();
            annot::Expr::Tuple(occurs)
        }

        flat::Expr::TupleField(tuple_id, idx) => {
            let tuple_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *tuple_id, fut_ty);
            annot::Expr::TupleField(tuple_occur, *idx)
        }

        flat::Expr::WrapVariant(variant_tys, variant_id, content) => todo!(),

        flat::Expr::UnwrapVariant(variant_id, wrapped) => {
            let content_ty =
                if let annot::Type::Variants(variant_tys) = &**ctx.local_binding(*wrapped) {
                    &variant_tys[variant_id]
                } else {
                    unreachable!();
                };

            let wrapped_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *wrapped, fut_ty);

            // annot::Expr::UnwrapVariant(variant_id, content_occur)
            todo!()
        }

        flat::Expr::WrapBoxed(content, item_ty) => todo!(),

        flat::Expr::UnwrapBoxed(wrapped, item_ty) => todo!(),

        flat::Expr::WrapCustom(custom_id, content) => {
            let annot::Type::Custom(fut_custom_id, modes, lts) = fut_ty else {
                unreachable!();
            };
            assert_eq!(custom_id, fut_custom_id);
            todo!()
        }

        flat::Expr::UnwrapCustom(custom_id, wrapped) => {
            // let wrapped_fresh = instantiate_type(customs, constrs, customs[custom_id].content);
            todo!()
        }

        flat::Expr::Intrinsic(intr, arg) => todo!(),

        flat::Expr::ArrayOp(flat::ArrayOp::Get(_, arr_id, idx_id)) => {
            let idx_occur = instantiate_occur_int(&ctx, *idx_id);

            let annot::Type::Array(mode, lt, item_ty, overlay) = &**ctx.local_binding(*arr_id)
            else {
                unreachable!();
            };
            let ret_ty = apply_overlay(item_ty, overlay);
            mode_bind(constrs, &ret_ty, fut_ty);

            // let item_ty = instantiate_type(customs, constrs, item_ty);

            // annot::Expr::ArrayOp(annot::ArrayOp::Get(item_ty, arr_occur, idx_occur))
            todo!()
        }

        flat::Expr::ArrayOp(flat::ArrayOp::Extract(item_ty, arr_id, idx_id)) => {
            // TODO: apply overlay
            let annot::Type::HoleArray(mode, _, item_ty, _) = fut_ty else {
                unreachable!();
            };
            constrs.force_owned(*mode);

            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, fut_ty);
            let idx_occur = instantiate_occur_int(&ctx, *idx_id);
            todo!()
            // let item_ty = instantiate_type(customs, constrs, item_ty);
            // annot::Expr::ArrayOp(annot::ArrayOp::Extract(item_ty, arr_occur, idx_occur))
        }

        flat::Expr::ArrayOp(flat::ArrayOp::Len(item_ty, arr_id)) => {
            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, fut_ty);
            let item_ty = instantiate_type_unused(customs, constrs, item_ty);
            annot::Expr::ArrayOp(annot::ArrayOp::Len(item_ty, arr_occur))
        }

        flat::Expr::ArrayOp(flat::ArrayOp::Push(item_ty, arr_id, item_id)) => {
            let annot::Type::Array(mode, _, _, _) = fut_ty else {
                unreachable!();
            };
            constrs.force_owned(*mode);

            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, fut_ty);
            let item_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(1), *item_id, fut_ty);
            let item_ty = instantiate_type_unused(customs, constrs, item_ty);
            annot::Expr::ArrayOp(annot::ArrayOp::Push(item_ty, arr_occur, item_occur))
        }

        flat::Expr::ArrayOp(flat::ArrayOp::Pop(item_ty, arr_id)) => {
            let annot::Type::Tuple(ret_items) = fut_ty else {
                unreachable!();
            };
            assert_eq!(ret_items.len(), 2);

            let annot::Type::Array(mode, _, fut_item_ty, _) = &ret_items[0] else {
                unreachable!();
            };
            constrs.force_owned(*mode);

            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, fut_ty);
            let item_ty = instantiate_type_unused(customs, constrs, item_ty);
            annot::Expr::ArrayOp(annot::ArrayOp::Pop(item_ty, arr_occur))
        }

        flat::Expr::ArrayOp(flat::ArrayOp::Replace(item_ty, hole_id, item_id)) => {
            let annot::Type::Array(mode, _, _, _) = fut_ty else {
                unreachable!();
            };
            constrs.force_owned(*mode);

            let hole_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *hole_id, fut_ty);
            let item_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(1), *item_id, fut_ty);
            let item_ty = instantiate_type_unused(customs, constrs, item_ty);
            annot::Expr::ArrayOp(annot::ArrayOp::Replace(item_ty, hole_occur, item_occur))
        }

        flat::Expr::ArrayOp(flat::ArrayOp::Reserve(item_ty, arr_id, cap_id)) => {
            let annot::Type::Array(mode, _, _, _) = fut_ty else {
                unreachable!();
            };
            constrs.force_owned(*mode);

            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, fut_ty);
            let cap_occur = instantiate_occur_int(&ctx, *cap_id);
            let item_ty = instantiate_type_unused(customs, constrs, item_ty);
            annot::Expr::ArrayOp(annot::ArrayOp::Reserve(item_ty, arr_occur, cap_occur))
        }

        flat::Expr::IoOp(flat::IoOp::Input) => {
            let annot::Type::Array(mode, _, item_ty, _) = fut_ty else {
                unreachable!();
            };
            constrs.force_owned(*mode);
            assert!(matches!(**item_ty, annot::Type::Num(NumType::Byte)));
            annot::Expr::IoOp(annot::IoOp::Input)
        }

        flat::Expr::IoOp(flat::IoOp::Output(arr_id)) => {
            let arr_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(0), *arr_id, fut_ty);
            annot::Expr::IoOp(annot::IoOp::Output(arr_occur))
        }

        flat::Expr::Panic(ret_ty, msg_id) => {
            let msg_occur =
                instantiate_occur(customs, &mut ctx, constrs, &path.seq(1), *msg_id, fut_ty);
            let ret_ty = instantiate_type_unused(customs, constrs, ret_ty);
            annot::Expr::Panic(ret_ty, msg_occur)
        }

        flat::Expr::ArrayLit(item_ty, item_ids) => {
            let annot::Type::Array(mode, _, _, _) = fut_ty else {
                unreachable!();
            };
            constrs.force_owned(*mode);

            let item_occurs = item_ids
                .iter()
                .enumerate()
                .map(|(i, item_id)| {
                    instantiate_occur(customs, &mut ctx, constrs, &path.seq(i), *item_id, fut_ty)
                })
                .collect();

            let item_ty = instantiate_type_unused(customs, constrs, item_ty);
            annot::Expr::ArrayLit(item_ty, item_occurs)
        }

        flat::Expr::BoolLit(val) => annot::Expr::BoolLit(*val),

        flat::Expr::ByteLit(val) => annot::Expr::ByteLit(*val),

        flat::Expr::IntLit(val) => annot::Expr::IntLit(*val),

        flat::Expr::FloatLit(val) => annot::Expr::FloatLit(*val),
    };

    (expr_annot, ctx.into_updates())
}

fn add_func_deps(deps: &mut BTreeSet<CustomFuncId>, expr: &flat::Expr) {
    match expr {
        flat::Expr::Call(_, other, _) => {
            deps.insert(*other);
        }
        flat::Expr::Branch(_, cases, _) => {
            for (_, body) in cases {
                add_func_deps(deps, body);
            }
        }
        flat::Expr::LetMany(bindings, _) => {
            for (_, rhs) in bindings {
                add_func_deps(deps, rhs);
            }
        }
        _ => {}
    }
}

fn instantiate_overlay(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    constrs: &mut ConstrGraph,
    ty: &anon::Type,
) -> SolverOverlay {
    match ty {
        anon::Type::Bool => Overlay::Bool,
        anon::Type::Num(num_ty) => Overlay::Num(*num_ty),
        anon::Type::Tuple(tys) => Overlay::Tuple(
            tys.iter()
                .map(|ty| instantiate_overlay(customs, constrs, ty))
                .collect(),
        ),
        anon::Type::Variants(tys) => {
            Overlay::Variants(tys.map_refs(|_, ty| instantiate_overlay(customs, constrs, ty)))
        }
        anon::Type::Custom(id) => annot::Overlay::Custom(
            *id,
            customs[*id]
                .overlay_params
                .iter()
                .map(|param| (*param, constrs.fresh_var()))
                .collect(),
        ),
        anon::Type::Array(_) => Overlay::Array(constrs.fresh_var()),
        anon::Type::HoleArray(_) => Overlay::HoleArray(constrs.fresh_var()),
        anon::Type::Boxed(_) => Overlay::Boxed(constrs.fresh_var()),
    }
}

fn instantiate_type(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    constrs: &mut ConstrGraph,
    fresh_lt: &mut impl FnMut() -> Lt,
    ty: &anon::Type,
) -> SolverType {
    match ty {
        anon::Type::Bool => annot::Type::Bool,
        anon::Type::Num(num_ty) => annot::Type::Num(*num_ty),
        anon::Type::Tuple(tys) => annot::Type::Tuple(
            tys.iter()
                .map(|ty| instantiate_type(customs, constrs, fresh_lt, ty))
                .collect(),
        ),
        anon::Type::Variants(tys) => annot::Type::Variants(
            tys.map_refs(|_, ty| instantiate_type(customs, constrs, fresh_lt, ty)),
        ),
        anon::Type::Custom(id) => annot::Type::Custom(
            *id,
            IdVec::from_count_with(customs[*id].num_mode_params, |_| constrs.fresh_var()),
            IdVec::from_count_with(customs[*id].num_lt_params, |_| fresh_lt()),
        ),
        anon::Type::Array(content_ty) => annot::Type::Array(
            constrs.fresh_var(),
            fresh_lt(),
            Box::new(instantiate_type(customs, constrs, fresh_lt, content_ty)),
            instantiate_overlay(customs, constrs, content_ty),
        ),
        anon::Type::HoleArray(content_ty) => annot::Type::HoleArray(
            constrs.fresh_var(),
            fresh_lt(),
            Box::new(instantiate_type(customs, constrs, fresh_lt, content_ty)),
            instantiate_overlay(customs, constrs, content_ty),
        ),
        anon::Type::Boxed(content_ty) => annot::Type::Boxed(
            constrs.fresh_var(),
            fresh_lt(),
            Box::new(instantiate_type(customs, constrs, fresh_lt, content_ty)),
            instantiate_overlay(customs, constrs, content_ty),
        ),
    }
}

// Replaces parameters with fresh variables from the constraint graph.
fn instantiate_type_unused(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    constrs: &mut ConstrGraph,
    ty: &anon::Type,
) -> SolverType {
    instantiate_type(customs, constrs, &mut || Lt::Empty, ty)
}

fn lt_equiv(ty1: &annot::Type<ModeVar, Lt>, ty2: &annot::Type<ModeVar, Lt>) -> bool {
    ty1.items()
        .zip(ty2.items())
        .map(
            |_, _, ((_, lts1), (_, lts2))| lts1 == lts2,
            |_, ((_, lt1), (_, lt2))| lt1 == lt2,
            |_, _, _| true,
            |_, _| true,
        )
        .fold(true, |a, b| a && b)
}

fn instantiate_scc(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    funcs: &IdVec<CustomFuncId, flat::FuncDef>,
    funcs_annot: &IdMap<CustomFuncId, annot::FuncDef>,
    scc: Scc<CustomFuncId>,
) -> SolverScc {
    match scc.kind {
        SccKind::Acyclic | SccKind::Cyclic => {
            // TODO: if the SCC is acyclic, we can skip lifetime fixed point iteration

            let mut constrs = ConstrGraph::new(); // TODO: when can we clear this?
            let mut next_lt = Count::new();

            let mut arg_tys = {
                scc.nodes
                    .iter()
                    .map(|id| {
                        (
                            *id,
                            instantiate_type_unused(customs, &mut constrs, &funcs[id].arg_type),
                        )
                    })
                    .collect::<BTreeMap<_, _>>()
            };

            let ret_tys = {
                scc.nodes
                    .iter()
                    .map(|id| {
                        (
                            *id,
                            instantiate_type(
                                customs,
                                &mut constrs,
                                &mut || Lt::var(next_lt.inc()),
                                &funcs[id].ret_type,
                            ),
                        )
                    })
                    .collect::<BTreeMap<_, _>>()
            };

            let bodies = loop {
                let mut new_arg_tys = BTreeMap::new();
                let mut bodies = BTreeMap::new();
                let assumptions = SignatureAssumptions {
                    known_defs: &funcs_annot,
                    pending_args: &arg_tys,
                    pending_rets: &ret_tys,
                };

                for id in scc.nodes {
                    let func = &funcs[id];

                    let mut ctx = ImmutContext::new();
                    let arg_ty = instantiate_type_unused(customs, &mut constrs, &func.arg_type);
                    let arg_id = ctx.add_local(Rc::new(arg_ty.clone())); // TODO: don't clone; use RC everywhere
                    assert_eq!(arg_id, flat::ARG_LOCAL);

                    let mut scopes = LocalContext::new();
                    let arg_id = scopes.add_local(annot::Path::root().seq(1));
                    assert_eq!(arg_id, flat::ARG_LOCAL);

                    let (expr, updates) = instantiate_expr(
                        customs,
                        assumptions,
                        &mut constrs,
                        &ctx,
                        &mut scopes,
                        // Should end before `scopes[arg_id]`
                        annot::Path::root().seq(0),
                        &ret_tys[id],
                        &func.body,
                    );
                    bodies.insert(*id, expr);

                    new_arg_tys.insert(
                        *id,
                        updates
                            .local_binding(flat::ARG_LOCAL)
                            .map(|ty| (**ty).clone())
                            .unwrap_or(arg_ty),
                    );
                }

                if new_arg_tys
                    .values()
                    .zip_eq(arg_tys.values())
                    .all(|(new, old)| lt_equiv(new, old))
                {
                    break bodies;
                }

                arg_tys = new_arg_tys;
            };

            SolverScc {
                sigs: arg_tys
                    .into_iter()
                    .zip(ret_tys.into_values())
                    .map(|((id, arg_type), ret_type)| (id, SolverSignature { arg_type, ret_type }))
                    .collect(),
                bodies,
                constrs,
            }
        }
    }
}

fn find_external_vars(sig: &SolverSignature) -> BTreeSet<ModeVar> {
    let mut params = BTreeSet::new();
    let params_st = state(&mut params);

    let record = |ty: &SolverType| {
        ty.items().for_each(
            |_, _, (ms, _)| {
                params_st.get().extend(ms.values().cloned());
            },
            |_, (m, _)| {
                params_st.get().insert(*m);
            },
            |_, _, ms| {
                params_st.get().extend(ms.values().cloned());
            },
            |_, m| {
                params_st.get().insert(*m);
            },
        );
    };

    record(&sig.arg_type);
    record(&sig.ret_type);
    params
}

struct Params(IdVec<EquivClass, ModeParam>);

fn extract_mode(solution: &Solution, params: &Params, m: &Mode<ModeVar>) -> Mode<ModeParam> {
    match m {
        Mode::Owned => Mode::Owned,
        Mode::Borrowed => Mode::Borrowed,
        Mode::Param(m) => Mode::Param(params.0[solution.classes.class(*m)]),
    }
}

fn extract_type(
    solution: &Solution,
    params: &Params,
    ty: &SolverType,
) -> annot::Type<Mode<ModeParam>, Lt> {
    let subst =
        |m: ModeVar| extract_mode(solution, params, &solution.modes[solution.classes.class(m)]);

    ty.items()
        .map(
            |_, _, (ms, lts)| (ms.map_refs(|_, m| subst(*m)), lts.clone()),
            |_, (m, lt)| (subst(*m), lt.clone()),
            |_, _, ms| ms.iter().map(|(id, m)| (*id, subst(*m))).collect(),
            |_, m| subst(*m),
        )
        .collect()
}

fn extract_constrs(solution: &Solution, params: &Params) -> Vec<annot::ModeConstr> {
    solution
        .iter_lte_constrs()
        .map(|(a, b)| annot::ModeConstr(params.0[a], params.0[b]))
        .collect()
}

fn extract_occur(
    solution: &Solution,
    params: &Params,
    occur: &SolverOccur,
) -> annot::Occur<Mode<ModeParam>, Lt> {
    annot::Occur {
        local: occur.local,
        ty: extract_type(solution, params, &occur.ty),
    }
}

fn extract_condition(
    solution: &Solution,
    params: &Params,
    cond: &SolverCondition,
) -> annot::Condition<Mode<ModeParam>, Lt> {
    match cond {
        annot::Condition::Any => annot::Condition::Any,
        annot::Condition::Tuple(conds) => annot::Condition::Tuple(
            conds
                .iter()
                .map(|cond| extract_condition(solution, params, cond))
                .collect(),
        ),
        annot::Condition::Variant(id, cond) => {
            annot::Condition::Variant(*id, Box::new(extract_condition(solution, params, cond)))
        }
        annot::Condition::Boxed(cond, ty) => annot::Condition::Boxed(
            Box::new(extract_condition(solution, params, cond)),
            extract_type(solution, params, ty),
        ),
        annot::Condition::Custom(id, cond) => {
            annot::Condition::Custom(*id, Box::new(extract_condition(solution, params, cond)))
        }
        annot::Condition::BoolConst(v) => annot::Condition::BoolConst(*v),
        annot::Condition::ByteConst(v) => annot::Condition::ByteConst(*v),
        annot::Condition::IntConst(v) => annot::Condition::IntConst(*v),
        annot::Condition::FloatConst(v) => annot::Condition::FloatConst(*v),
    }
}

fn extract_expr(
    solution: &Solution,
    params: &Params,
    expr: &SolverExpr,
) -> annot::Expr<Mode<ModeParam>, Lt> {
    use annot::Expr as E;
    match expr {
        E::Local(occur) => E::Local(extract_occur(solution, params, occur)),
        E::Call(purity, func, arg) => E::Call(*purity, *func, extract_occur(solution, params, arg)),
        E::Branch(discrim, branches, ret_ty) => E::Branch(
            extract_occur(solution, params, discrim),
            branches
                .iter()
                .map(|(cond, body)| {
                    (
                        extract_condition(solution, params, cond),
                        extract_expr(solution, params, body),
                    )
                })
                .collect(),
            extract_type(solution, params, ret_ty),
        ),
        E::LetMany(bindings, result) => E::LetMany(
            bindings
                .iter()
                .map(|(ty, expr)| {
                    (
                        extract_type(solution, params, ty),
                        extract_expr(solution, params, expr),
                    )
                })
                .collect(),
            extract_occur(solution, params, result),
        ),
        E::Tuple(items) => E::Tuple(
            items
                .iter()
                .map(|occur| extract_occur(solution, params, occur))
                .collect(),
        ),
        E::TupleField(tup, idx) => E::TupleField(extract_occur(solution, params, tup), *idx),
        E::WrapVariant(variant_tys, id, content) => E::WrapVariant(
            variant_tys.map_refs(|_, ty| extract_type(solution, params, ty)),
            *id,
            extract_occur(solution, params, content),
        ),
        E::UnwrapVariant(id, wrapped) => {
            E::UnwrapVariant(*id, extract_occur(solution, params, wrapped))
        }
        E::WrapBoxed(content, item_ty) => E::WrapBoxed(
            extract_occur(solution, params, content),
            extract_type(solution, params, item_ty),
        ),
        E::UnwrapBoxed(wrapped, item_ty) => E::UnwrapBoxed(
            extract_occur(solution, params, wrapped),
            extract_type(solution, params, item_ty),
        ),
        E::WrapCustom(id, content) => E::WrapCustom(*id, extract_occur(solution, params, content)),
        E::UnwrapCustom(id, wrapped) => {
            E::UnwrapCustom(*id, extract_occur(solution, params, wrapped))
        }
        E::Intrinsic(intr, arg) => E::Intrinsic(*intr, extract_occur(solution, params, arg)),
        E::ArrayOp(annot::ArrayOp::Get(item_ty, arr, idx, out_ty)) => {
            E::ArrayOp(annot::ArrayOp::Get(
                extract_type(solution, params, item_ty),
                extract_occur(solution, params, arr),
                extract_occur(solution, params, idx),
                extract_type(solution, params, out_ty),
            ))
        }
        E::ArrayOp(annot::ArrayOp::Extract(item_ty, arr, idx)) => {
            E::ArrayOp(annot::ArrayOp::Extract(
                extract_type(solution, params, item_ty),
                extract_occur(solution, params, arr),
                extract_occur(solution, params, idx),
            ))
        }
        E::ArrayOp(annot::ArrayOp::Len(item_ty, arr)) => E::ArrayOp(annot::ArrayOp::Len(
            extract_type(solution, params, item_ty),
            extract_occur(solution, params, arr),
        )),
        E::ArrayOp(annot::ArrayOp::Push(item_ty, arr, item)) => E::ArrayOp(annot::ArrayOp::Push(
            extract_type(solution, params, item_ty),
            extract_occur(solution, params, arr),
            extract_occur(solution, params, item),
        )),
        E::ArrayOp(annot::ArrayOp::Pop(item_ty, arr)) => E::ArrayOp(annot::ArrayOp::Pop(
            extract_type(solution, params, item_ty),
            extract_occur(solution, params, arr),
        )),
        E::ArrayOp(annot::ArrayOp::Replace(item_ty, hole, item)) => {
            E::ArrayOp(annot::ArrayOp::Replace(
                extract_type(solution, params, item_ty),
                extract_occur(solution, params, hole),
                extract_occur(solution, params, item),
            ))
        }
        E::ArrayOp(annot::ArrayOp::Reserve(item_ty, arr, cap)) => {
            E::ArrayOp(annot::ArrayOp::Reserve(
                extract_type(solution, params, item_ty),
                extract_occur(solution, params, arr),
                extract_occur(solution, params, cap),
            ))
        }
        E::IoOp(annot::IoOp::Input) => E::IoOp(annot::IoOp::Input),
        E::IoOp(annot::IoOp::Output(arr)) => {
            E::IoOp(annot::IoOp::Output(extract_occur(solution, params, arr)))
        }
        E::Panic(ty, msg) => E::Panic(
            extract_type(solution, params, ty),
            extract_occur(solution, params, msg),
        ),
        E::ArrayLit(item_ty, items) => E::ArrayLit(
            extract_type(solution, params, item_ty),
            items
                .iter()
                .map(|occur| extract_occur(solution, params, occur))
                .collect(),
        ),
        E::BoolLit(v) => E::BoolLit(*v),
        E::ByteLit(v) => E::ByteLit(*v),
        E::IntLit(v) => E::IntLit(*v),
        E::FloatLit(v) => E::FloatLit(*v),
    }
}

fn solver_type_from_sig(
    constrs: &mut ConstrGraph,
    sig_constrs: &Vec<ModeConstr>,
    ty: &annot::Type<Mode<ModeParam>, Lt>,
) -> SolverType {
    let mut mapping = BTreeMap::new();

    let constrs_st = state(constrs);
    let mapping_st = state(&mut mapping);

    let intro_var = |m: &Mode<ModeParam>| match m {
        Mode::Owned => {
            let var = constrs_st.get().fresh_var();
            constrs_st.get().force_owned(var);
            var
        }
        Mode::Borrowed => panic!("borrowed mode in signature"),
        Mode::Param(param) => {
            let var = constrs_st.get().fresh_var();
            mapping_st.get().insert_vacant(*param, var);
            var
        }
    };

    let solver_ty = ty
        .items()
        .map(
            |_, _, (ms, lts)| (ms.map_refs(|_, m| intro_var(m)), lts.clone()),
            |_, (m, lt)| (intro_var(m), lt.clone()),
            |_, _, ms| ms.iter().map(|(id, m)| (*id, intro_var(m))).collect(),
            |_, m| intro_var(m),
        )
        .collect();

    for ModeConstr(a, b) in sig_constrs {
        constrs_st.get().force_lte(mapping[a], mapping[b]);
    }

    solver_ty
}

#[derive(Clone, Debug)]
struct SolverSignature {
    arg_type: SolverType,
    ret_type: SolverType,
}

#[derive(Clone, Copy, Debug)]
struct SignatureAssumptions<'a> {
    known_defs: &'a IdMap<CustomFuncId, annot::FuncDef>,
    pending_args: &'a BTreeMap<CustomFuncId, SolverType>,
    pending_rets: &'a BTreeMap<CustomFuncId, SolverType>,
}

impl<'a> SignatureAssumptions<'a> {
    fn sig_of(
        &self,
        constrs: &mut ConstrGraph,
        id: CustomFuncId,
    ) -> (annot::Type<ModeVar, Lt>, annot::Type<ModeVar, Lt>) {
        self.known_defs.get(id).map_or_else(
            || {
                (
                    self.pending_args[&id].clone(),
                    self.pending_rets[&id].clone(),
                )
            },
            |def| {
                (
                    solver_type_from_sig(constrs, &def.sig_constrs, &def.sig.arg_type),
                    solver_type_from_sig(constrs, &def.sig_constrs, &def.sig.ret_type),
                )
            },
        )
    }
}

struct SolverScc {
    sigs: BTreeMap<CustomFuncId, SolverSignature>,
    bodies: BTreeMap<CustomFuncId, annot::Expr<ModeVar, Lt>>,
    constrs: ConstrGraph,
}

fn solve_scc(
    customs: &IdVec<CustomTypeId, annot::TypeDef>,
    funcs: &IdVec<CustomFuncId, flat::FuncDef>,
    funcs_annot: &mut IdMap<CustomFuncId, annot::FuncDef>,
    scc: Scc<CustomFuncId>,
) {
    let instantiated = instantiate_scc(customs, funcs, funcs_annot, scc);

    for func_id in scc.nodes {
        let sig = &instantiated.sigs[func_id];
        let external = find_external_vars(sig);
        let solution = instantiated.constrs.solve_for(&external);

        let num_mode_params = count_mode_params(customs, sig.arg_type.items())
            + count_mode_params(customs, sig.ret_type.items());
        let num_lt_params = count_lt_params(customs, sig.arg_type.items())
            + count_lt_params(customs, sig.ret_type.items());

        let params = Params(solution.modes.map_refs(|id, _| ModeParam(id.0)));
        let func = &funcs[func_id];
        funcs_annot.insert_vacant(
            *func_id,
            annot::FuncDef {
                purity: func.purity,
                sig: annot::Signature {
                    num_mode_params: Count::from_value(num_mode_params),
                    num_lt_params: Count::from_value(num_lt_params),
                    arg_type: extract_type(&solution, &params, &sig.arg_type),
                    ret_type: extract_type(&solution, &params, &sig.ret_type),
                },
                sig_constrs: extract_constrs(&solution, &params),
                body: extract_expr(&solution, &params, &instantiated.bodies[func_id]),
                profile_point: func.profile_point.clone(),
            },
        );
    }
}

fn convert_type_sccs(
    orig: &IdVec<CustomTypeSccId, old_graph::Scc<CustomTypeId>>,
) -> Sccs<CustomTypeSccId, CustomTypeId> {
    let mut sccs = Sccs::new();
    for (id, scc) in orig {
        let push_id = match scc {
            old_graph::Scc::Acyclic(node) => sccs.push_acyclic_component(*node),
            old_graph::Scc::Cyclic(nodes) => sccs.push_cyclic_component(nodes),
        };
        debug_assert_eq!(push_id, id);
    }
    sccs
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Strategy {
    Aggressive,
    AlwaysOwned,        // similar to "Perceus"
    OnlyTrivialBorrows, // similar to "Immutable Beans"
}

pub fn annot_modes(
    program: &flat::Program,
    _strat: Strategy,
    progress: impl ProgressLogger,
) -> annot::Program {
    let mut progress = progress.start_session(Some(program.funcs.len()));

    let customs = parameterize_customs(&program.custom_types);

    #[id_type]
    struct FuncSccId(usize);

    let sccs: Sccs<FuncSccId, _> = find_components(program.funcs.count(), |id| {
        let mut deps = BTreeSet::new();
        add_func_deps(&mut deps, &program.funcs[id].body);
        deps
    });

    let mut funcs_annot = IdMap::new();
    for (_, scc) in &sccs {
        solve_scc(&customs, &program.funcs, &mut funcs_annot, scc);
        progress.update(scc.nodes.len());
    }

    let funcs_annot = funcs_annot.to_id_vec(program.funcs.count());

    progress.finish();

    annot::Program {
        mod_symbols: program.mod_symbols.clone(),
        custom_types: annot::CustomTypes {
            types: customs,
            sccs: convert_type_sccs(&program.custom_types.sccs),
        },
        custom_type_symbols: program.custom_type_symbols.clone(),
        funcs: funcs_annot,
        func_symbols: program.func_symbols.clone(),
        profile_points: program.profile_points.clone(),
        main: program.main,
    }
}
